...
burglaries_census_merged_grouped |>
  ggplot(aes(x=median_income, y=count_per_1000_residents)) +
  geom_point()
```
...
Possibly an exponential decay trend?
...
```{r}
burglaries_census_merged_grouped |>
  ggplot(aes(x=Log_median_income, y=count_per_1000_residents)) +
  geom_point()
```
...
```{r}
burglaries_census_merged_grouped_lm <- lm("count_per_1000_residents ~ median_income", data = burglaries_census_merged_grouped)
```


```{r}
summary(burglaries_census_merged_grouped_lm)
```
```{r}
coef(burglaries_census_merged_grouped_lm)
```
